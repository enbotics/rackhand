...
export const executePutawayTool = tool({
  ...
  description:
    "Call this tool immediately for an explicit putaway or CHECKED_OUT-bin return; never ask for a photo in chat. After client HITL it opens the comparison flow. Production uses one fresh Pi frame; Simulation uses the next controlled fixture through the same Gemini and decision gates. Strictly over 60% confidence is required: equal proceeds and any verified mismatch updates automatically after movement; foreign objects, low confidence, occlusion or capacity overflow require the appropriate next capture. The gantry runs only after verification. ok:true means movement and database commit completed.",
