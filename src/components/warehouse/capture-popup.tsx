...
export function CapturePopup({
  ...
  return (
    <Modal
      ...
      dismissible={dismissible && exit === null}
    ...
    >
      <div className="overflow-hidden rounded-2xl border border-line bg-bg-elevated">
        <div className="relative aspect-video bg-[#061018]">
            ...
            <div className="absolute inset-0 grid place-items-center px-10 text-center">
              <div>
                ...
                <p className="mt-3 text-sm leading-relaxed text-ink-muted">
                  The next curated fixture will be analyzed through the same
                  Gemini verification used in production.
                </p>
              </div>
            </div>
          ) : (
          ...
          {!simulation && !previewReady && (
            <div className="absolute inset-0 grid place-items-center px-8 text-center text-xs leading-relaxed text-ink-muted">
              Connecting to the live Pi camera… Capture unlocks with the first
              frame.
            </div>
          )}
          <div
            className={`absolute left-3 top-3 flex items-center gap-2 rounded-full border bg-black/65 px-3 py-1.5 text-[10px] font-semibold tracking-[0.18em] backdrop-blur ${simulation ? "border-amber-300/30 text-amber-200" : "border-success/30 text-success"}`}
          >
            <span className="relative flex h-2 w-2">
              <span
                className={`absolute inline-flex h-full w-full animate-ping rounded-full opacity-40 ${simulation ? "bg-amber-300" : "bg-success"}`}
              />
              <span
                className={`relative inline-flex h-2 w-2 rounded-full ${simulation ? "bg-amber-300" : "bg-success"}`}
              />
            </span>
        ...
        <div className="p-5">
          <p className="text-sm font-semibold text-ink">
            {simulation ? "Simulation ready" : "Frame ready for verification"}
          </p>
          ...
          <button
            ...
            onClick={() => finish(true)}
            disabled={
              disabled || (!simulation && !previewReady) || exit !== null
            }
      ...
      {disabled && (
        <p className="mt-3 text-xs text-ink-muted">
          Capture unlocks when the gantry stops.
        </p>
      )}
      {error && (
        <div className="mt-3">
