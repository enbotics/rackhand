...
import {
  SIMULATION_ELIGIBLE_BINS,
  SIMULATION_LOCK_REASON,
} from "@/lib/warehouse/simulation-policy";
  ...
  if (mode === "PRODUCTION" || !locked) {
    return (
      <span
        title={
          mode === "PRODUCTION"
            ? "Production mode sends commands to real Klipper hardware."
            : "Gantry movement is simulated."
        }
        className="rounded-full border border-warn/40 bg-warn-soft px-3 py-1.5 font-mono text-[9px] uppercase tracking-wider text-warn"
      >
        {mode === "PRODUCTION" ? "Production · Klipper" : "Gantry Simulation"}
      </span>
    );
  }
  ...
  return (
    <>
      ...
      >
        ...
        <span
          aria-hidden="true"
          className="grid size-3.5 place-items-center rounded-full border border-warn/50 text-[9px] normal-case"
        >
          i
        </span>
      </button>
        ...
        createPortal(
          ...
          >
            <div className="flex items-center justify-between gap-3 border-b border-line pb-3">
              <h2 className="font-mono text-[11px] font-semibold uppercase tracking-wider text-warn">
              ...
              <button
                ...
                onClick={closeGuide}
                className="grid size-6 cursor-pointer place-items-center rounded text-ink-muted hover:bg-surface hover:text-ink"
              >
                ×
              </button>
            </div>
            ...
            <div className="mt-3 flex gap-2">
              {SIMULATION_ELIGIBLE_BINS.map((bin) => (
                <span
                  key={bin}
                  className="rounded-md border border-accent/30 bg-accent-soft px-2 py-1 font-mono text-[11px] text-accent"
                >
                  {bin}
                </span>
              ))}
            ...
            <p className="mt-3 text-[11px] leading-relaxed text-ink-faint">
              Approve the requested move in the chat. Return the checked-out bin
              before requesting another. Images are simulation fixtures; B1-02
              uses a reference photo. Demo parts without a configured item
              weight keep their recorded quantity.
            </p>
            <div className="mt-4 flex justify-end border-t border-line pt-3">
              <button
                type="button"
                onClick={closeGuide}
                className="cursor-pointer rounded-lg border border-warn/40 bg-warn-soft px-4 py-2 text-xs font-medium text-warn transition-colors hover:border-warn focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-warn"
              >
                Got it
              </button>
            </div>
