...
vi.mock("@/components/warehouse/session", () => ({ useWarehouseSession: () => ({ totals: null, gantry: { mode: "SIMULATION" }, approval: null, identification: null }) }));
...
describe("simulation guide", () => {
  it("opens the floating dialog by default with the lock, allowed bins and prompts", async () => {
    ...
    const { container } = render(<section style={{ overflow: "hidden" }}><AuditCaptureModeToggle /></section>);
    const trigger = screen.getByRole("button", { name: "Simulation locked — view demo guide" });
    const guide = await screen.findByRole("dialog", { name: "Simulation demo guide" });
    expect(guide.getAttribute("aria-modal")).toBe("false");
    expect(container.contains(guide)).toBe(false);
    expect(screen.getByText(/Prod mode can trigger real hardware/)).toBeTruthy();
    expect(screen.getByText("B1-01")).toBeTruthy();
    expect(screen.getByText("B1-02")).toBeTruthy();
    expect(screen.getByText("“Bring me bin B1-01”")).toBeTruthy();
    expect(screen.getByText("“Bring me bin B1-02”")).toBeTruthy();
    expect(trigger.getAttribute("aria-expanded")).toBe("true");
    expect(fetch).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: "Close simulation guide" }));
    expect(screen.queryByRole("dialog", { name: "Simulation demo guide" })).toBeNull();
