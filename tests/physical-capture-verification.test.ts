...
const fixture = vi.hoisted(() => {
  ...
  const updateCapture = vi.fn(
    async ({
      ...
      if (
        (allowed && !allowed.includes(capture.status as string)) ||
        (where.attempt !== undefined && capture.attempt !== where.attempt)
      )
        return { count: 0 };
...
vi.mock("@/lib/warehouse/db", () => ({
  prisma: {
    ...fixture.database,
    $transaction: vi.fn(
      async (callback: (tx: typeof fixture.database) => unknown) =>
...
vi.mock("@/lib/camera/storage", () => ({
  readCameraCapture: fixture.readCameraCapture,
}));
vi.mock("@/lib/warehouse/bin-inspection-service", async () => ({
  ...(await vi.importActual<
    typeof import("@/lib/warehouse/bin-inspection-service")
  >("@/lib/warehouse/bin-inspection-service")),
  inspectBinImage: fixture.inspect,
}));
vi.mock("@/lib/warehouse/putaway-recovery-service", () => ({
  recoverAbandonedPutaways: async () => ({}),
  putawayInactivityTimeoutMs: () => 240_000,
}));
vi.mock("@/lib/agents/request-context", () => ({
  getContextWorkflowSessionId: () => "session-1",
  getContextBrowserScenario: () => null,
}));

import {
  decidePutawayCapture,
  processPutawayCameraCapture,
  reanalyzePutawayCapture,
  requirePutawayVerification,
} from "@/lib/warehouse/putaway-verification";
...
describe("physical verification state machine", () => {
  ...
  it("recovers a valid retrieval photo rejected by an older camera deployment", async () => {
    ...
    Object.assign(fixture.capture, {
      ...
      evidenceUrl: "/saved.jpg",
      capturedAt: now,
      imageWidth: 100,
      imageHeight: 100,
      notes:
        "The saved photo could not be analyzed (This capture is stale or no longer pending.).",
    });
  ...
  it("uses the supported Gemini API model code rather than a display name", () => {
    expect(GEMINI_VISION_MODEL_ID).toBe("gemini-3.5-flash-lite");
  });
  ...
  it("publishes the capture and ready movement marker in one transaction", async () => {
    ...
    expect(
      fixture.database.movement.update.mock.invocationCallOrder[0],
    ).toBeLessThan(
  ...
  it("reanalyzes a saved frame after a model error without taking another photo", async () => {
    ...
    fixture.inspect.mockRejectedValueOnce(
      new Error("models/gemini-3.5 is not found"),
    );
    ...
    expect(
      (await reanalyzePutawayCapture("capture-1", "session-1")).outcome,
    ).toBe("REVIEW_DECREASE");
    expect(fixture.readCameraCapture).toHaveBeenCalledWith("camera-1");
  ...
  it("automatically accepts a trusted mismatch without a browser decision", async () => {
    ...
    await expect(
      decidePutawayCapture("capture-1", "ACCEPT", "session-1"),
  ...
  it("waits for human removal and retry when an unexpected object is present", async () => {
    ...
    await expect(
      decidePutawayCapture("capture-1", "AUTO_RETURN", "session-1"),
    ).rejects.toThrow();
